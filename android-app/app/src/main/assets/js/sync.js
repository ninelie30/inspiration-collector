...
async function webdavRequest(endpoint, settings) {
  // Android 原生桥接模式（绕过 CORS，国内 IP 直连坚果云）
  if (window.WebDAVBridge) {
    const body = JSON.stringify({
      username: settings.username,
      password: settings.password,
      server: settings.server || '',
    });
    const result = JSON.parse(window.WebDAVBridge.webdavRequest(endpoint, body));
    if (!result.ok) throw new Error(result.error || 'WebDAV error');
    return result;
  }
  // 标准模式：通过后端代理
...
async function webdavPushData(settings, data) {
  ...
  if (window.WebDAVBridge) {
    ...
    const result = JSON.parse(window.WebDAVBridge.webdavRequest('push', body));
    if (!result.ok) throw new Error(result.error || 'WebDAV error');
    return result;
  }
  // 标准模式
